    ...
    <>
      ...
      >
        <div className="flex items-center gap-1">
          ...
          {/* ChatGPT */}
          <button
            onClick={onToggleChatGPT}
            className="w-7 h-7 flex items-center justify-center rounded-lg transition-all"
            style={{
              background: isChatGPTOpen
                ? (isDark ? 'rgba(16,185,129,0.2)' : 'rgba(209,250,229,1)')
                : 'transparent',
              color: isChatGPTOpen ? '#10b981' : 'var(--text-muted)',
              border: isChatGPTOpen ? `1px solid ${isDark ? 'rgba(16,185,129,0.3)' : '#a7f3d0'}` : '1px solid transparent',
            }}
            onMouseEnter={e => { if (!isChatGPTOpen) { e.currentTarget.style.background = 'var(--surface-hover)'; e.currentTarget.style.color = 'var(--text-secondary)'; } }}
            onMouseLeave={e => { if (!isChatGPTOpen) { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = 'var(--text-muted)'; } }}
            aria-label="ChatGPT"
          >
            <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
